...
if __name__ == '__main__':
    Server = Handler('eoajt44toya9wcdtrcuytl7scfiu8c', 'k0xjsg5h8tpfo3gxfdhr47howmwm93', 'https://www.whitekeks.tk')

    loop = asyncio.get_event_loop()

    # loop.run_until_complete(Server.HookStream('Whitekekss', 'subscribe'))
    # input("To unsubscribe Connection press Enter")
    Subscriptions = loop.run_until_complete(Server.GETRequest(
        url='https://api.twitch.tv/helix/webhooks/subscriptions',
        params=None,
        headers={
            'Authorization': loop.run_until_complete(Server.getToken()),
            'Client-Id': Server.CLIENTID
        }
    ))
    print(Subscriptions)
    # loop.run_until_complete(Server.HookStream('Whitekekss', 'unsubscribe'))
    ...
    #         'Client-Id': Server.CLIENTID
    # }
    # User = loop.run_until_complete(Server.GETRequest(
    #     url='https://api.twitch.tv/helix/users',
    #     params={'login': "c4ndygg"},
    #     headers = headers
    # ))

    # UserID = User['data'][0]['id']
    # Video = loop.run_until_complete(Server.GETRequest(
